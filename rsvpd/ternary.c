/* ternary.c - Ternary Search Trees
   ...
   /*------------------------------------------------------------------------
    * Modification history : Pranjal Kumar Dutta, 2002
    *                        1) Replaced PTR macros with "void" everywhere.
    *                        2) Replaced PARAMS macros and done cleanup
    *                           of input parameters in functions. 
    *----------------------------------------------------------------------*/
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
void *
ternary_insert ( ternary_tree *root, 
                 const char *s, 
                 void *data, 
                 int replace)
{
...
void * 
ternary_search (const ternary_node *p, 
                const char *s)
{
...
static void *
ternary_recursivesearch (const ternary_node *p, 
                         const char *s)
{
  if (!p)
    return 0;
