#! /usr/bin/perl
##
## imi-cli/extract.pl.  Generated from extract.pl.in by configure.
##
...
## command.c
$ignore{'"no hostname [HOSTNAME]"'} = "ignore";
$ignore{'"hostname WORD"'} = "ignore";
$ignore{'"show startup-config"'} = "ignore";
## vty.c
$ignore{'"who"'} = "ignore";
$ignore{'"login"'} = "ignore";
$ignore{'"no login"'} = "ignore";
$ignore{'"line vty"'} = "ignore";
$ignore{'"show history"'} = "ignore";
$ignore{'"terminal monitor"'} = "ignore";
$ignore{'"terminal no monitor"'} = "ignore";
$ignore{'"no terminal monitor"'} = "ignore";
$ignore{'"exec-timeout <0-35791>"'} = "ignore";
$ignore{'"exec-timeout <0-35791> <0-2147483>"'} = "ignore";
$ignore{'"no exec-timeout"'} = "ignore";
$ignore{'"access-class WORD"'} = "ignore";
$ignore{'"no access-class [WORD]"'} = "ignore";
$ignore{'"ipv6 access-class WORD"'} = "ignore";
$ignore{'"no ipv6 access-class [WORD]"'} = "ignore";
$ignore{'"anonymous restricted"'} = "ignore";
$ignore{'"no anonymous restricted"'} = "ignore";
$ignore{'"service advanced-vty"'} = "ignore";
$ignore{'"no service advanced-vty"'} = "ignore";
## olsrd
$ignore{'"router olsr"'} = "ignore";
	...
	$cmd =~ s/\s+$//g;
        ...
        if ($file =~ /lib\/keychain\.c$/) {
            ...
            if ($defun_array[1] =~ m/ipv6/) {
                $protocol = "IMISH_RIPNGD|IMISH_OSPF6D|IMISH_BGPD|IMISH_ZEBRA";
            } else {
                $protocol = "IMISH_RIPD|IMISH_OSPFD|IMISH_BGPD|IMISH_ZEBRA";
            }
        }
        elsif ($file =~ /lib\/distribute\.c$/) {
            if ($defun_array[1] =~ m/ipv6/) {
                $protocol = "IMISH_RIPNGD";
            } else {
                $protocol = "IMISH_RIPD";
            }
        }
